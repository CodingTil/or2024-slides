...
\begin{frame}
\frametitle{Overview of Vanderbeck's Generic Branching Scheme}
\begin{columns}
\begin{column}{0.5\textwidth}
	\centering
	\begin{tikzpicture}
		...
		\only<2->{
			...
			\fill[blue, opacity=0.25] (B1) -- (B2) -- (B3) -- (B4) -- cycle;
			% Outline the blue area
			\draw[blue, thick] (B1) -- (B2) -- (B3) -- (B4) -- cycle;

			% Define the green area coordinates
			\coordinate (G1) at (2,1);
			\coordinate (G2) at (5,1);
...
\begin{column}{0.5\textwidth}
	...
	\item	create $\abs{S} + 1 = 3$ child nodes:
			\begin{enumerate}
			\item	${\color{blue} S_1 \coloneqq \{ \left( x_1, <, 1 \right) \}}$
			\item	${\color{OliveGreen} S_2 \coloneqq \{ \left( x_1, \geq, 1 \right), \left( x_2, <, 2 \right) \}}$
...
\item	to the \MP{} add constraint:
		\begin{equation*}
		\sum_{q \in Q'_j} \lambda_q \geq K_j
		\end{equation*}
...
\begin{frame}
\frametitle{Component Bound Branching Rule}
Again:
\begin{itemize}
\item	find a subset $Q'$ of columns using a component bound sequence $S$:
		...
		S \coloneqq \{ \left( x_i, \eta_i, v_i \right) \mid \eta_i \in \{\leq, \geq\}, v_i \in \mathbb{Z} \}
		\end{equation*}
\item	such that: \quad $\sum_{q \in Q'} \lambda_q^* = K \not\in \mathbb{Z}$
\end{itemize}
...
\item	create binary search tree:
		...
		\setlength{\abovedisplayskip}{0pt} \setlength{\abovedisplayshortskip}{0pt}
		\begin{equation}
		\sum_{q \in Q'} \lambda_q \geq \ceil{K} \quad \left[\gamma\right]
		\end{equation}
		\end{minipage}
		\end{multicols}
\pause
...
&& {\color{blue} y_i = 1} &{\color{blue} \Leftrightarrow x_i \leq v_i }& {\color{blue} \forall \left( x_i, \eta_i, v_i \right) \in S} \\
...
\begin{itemize}
\item	\GCG{} solver only allows adding original constraints, which are reformulated
...
\item	with the following modification to the pricing problem:
		...
		\end{equation*}
\pause
\item[$\rightarrow$]	new interface in \GCG{} to create and manage such constraints
\end{itemize}
\end{frame}
...
\item	How many Component Bounds are required?
\end{itemize}
\end{frame}
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/general/MostFractional/solve_status.svg}
\end{figure}
\end{frame}
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/general/MostFractional/times.svg}
\end{figure}
\end{frame}
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/general/MostFractional/outperforms_generic.svg}
\end{figure}
\end{frame}
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/bound_stats/avg_num_bounds.svg}
\end{figure}
\end{frame}
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/bound_stats/compbnd/avg_num_bounds_vs_depth.svg}
\end{figure}
\end{frame}
...
\begin{itemize}
\item	implemented a new interface for managing generic mastercuts in \GCG{}
\item	implemented a simple branching rule for component bound sequences using generic mastercuts
\item	Vanderbeck's Generic Branching Scheme outperforms the Component Bound Branching Rule
\item	implemenation of the Component Bound Branching Rule showcases the potential of the new interface
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/bound_stats/compbnd/depths_distribution.svg}
\end{figure}
\end{frame}
...
\centering
\begin{figure}
\includesvg[height=0.9\textheight]{graphics/slides/bound_stats/num_bounds.svg}
\end{figure}
\end{frame}
